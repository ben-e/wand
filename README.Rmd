...
wand_fit <- wand(mpg ~ cyl + disp + drat + qsec + vs + am + gear + carb +
                   s_mlp(hp) + s_mlp(wt) + s_mlp(hp, wt),
                 data = mtcars)

build_wand_graph(wand_fit) %>% 
  ggraph(layout = 'kk') +
...
The `wand` package also includes a few convenience functions for inspecting the fitted smooths. Currently, only 1d and 2d smooths are supported. Here is an example of a 2d contour.
